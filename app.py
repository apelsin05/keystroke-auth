...
from utils.agent_keystroke import compare_profiles, save_keystroke_sample, train_model
...
def two_fa():
    ...
    # --- KEYSTROKE SCORING + ORCHESTRATOR ---
    # Determinăm login_count actual pentru dispozitiv
    try:
    ...
    keystroke_score = 1.0  # default: enrollment sau fallback (nu penalizăm)

    if user and int(user['keystroke_enabled']) == 1:
        if device_login_count < ENROLLMENT_LOGINS:
            # Faza de enrollment: colectăm date, nu scorăm
            save_keystroke_sample(KEYSTROKES_CSV, user_id, device_id, login_id, ks_raw)
            # La ultimul login din enrollment, antrenăm modelul
            if device_login_count == ENROLLMENT_LOGINS - 1:
                train_model(KEYSTROKES_CSV, user_id, device_id)
        else:
            # Faza de scoring: comparăm cu profilul învățat
            keystroke_score = compare_profiles(KEYSTROKES_CSV, user_id, device_id, ks_raw)

    result   = decide(keystroke_score, ip_score)
    print(f"[DEBUG] Keystroke: {keystroke_score:.3f}, IP: {ip_score}, Decision: {result['decision']}")
    decision = result['decision']

    # Dacă decizia e re-enrollment, resetăm dispozitivul
    ...
    # Sesiune unlawful dacă scorul e sub prag dar 2FA a trecut
    if user and int(user['keystroke_enabled']) == 1 and device_login_count >= ENROLLMENT_LOGINS:
        if keystroke_score < 0.3:
    ...
    try:
        ...
        for _, row in df_user_devices.iterrows():
            ...
            devices.append({
                ...
                'enrolled':     enrolled,
                'progress_pct': min(int(login_count / ENROLLMENT_LOGINS * 100), 100),
            })
    except Exception:
        pass

    return render_template('dashboard.html', user=user, logins=logins, devices=devices,
                           enrollment_target=ENROLLMENT_LOGINS)
